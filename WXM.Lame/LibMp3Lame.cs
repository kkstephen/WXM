...
namespace WXM.Media
{
	...
	public class LibMp3Lame : IDisposable
	{
        ...
        {
            get { return File.Exists("libmp3lame.dll"); }
        }
        ...
        public LibMp3Lame()
		{ 
            this.context = NativeMethods.lame_init();
		...
		public VBRMode VBR
        {
            get { return NativeMethods.lame_get_VBR(context); }
        ...
        {
            get { return NativeMethods.lame_get_VBR_q(context); }
            set { setter(NativeMethods.lame_set_VBR_q, value); }
        }
		/// <summary>VBR quality level.  0 = highest, 9 = lowest</summary>
		public float VBRQuality
        {
            get { return NativeMethods.lame_get_VBR_quality(context); }
            set { setter(NativeMethods.lame_set_VBR_quality, value); }
        }
		/// <summary>ABR average bitrate</summary>
		public int VBRMeanBitrateKbps
        {
            get { return NativeMethods.lame_get_VBR_mean_bitrate_kbps(context); }
        ...
        {
            get { return NativeMethods.lame_get_VBR_min_bitrate_kbps(context); }
        ...
        {
            get { return NativeMethods.lame_get_VBR_max_bitrate_kbps(context); }
        ...
        {
            get { return NativeMethods.lame_get_VBR_hard_min(context); }
...
#pragma warning restore 1591
        ...
        {
            byte[] outBuffer = new byte[4096 * 8];
            byte[] inBuffer = new byte[4096 * 8];

            //short = int16         
            int count = 0;
            double total = 0;

            FileStream fs_in = new FileStream(wavfile, FileMode.Open);
             
            try
            {
                int length = (int)fs_in.Length;

                this.outStream = File.Create(mp3file);

                //skip wav header
                fs_in.Seek(44, SeekOrigin.Begin);

                while (total < length - 1)
                {
                    int dwRead = fs_in.Read(inBuffer, 0, inBuffer.Length);

                    if (dwRead > 0)
                    {
                        short[] samples = new short[inBuffer.Length];
                        Buffer.BlockCopy(inBuffer, 0, samples, 0, dwRead);

                        count = this.Write(samples, dwRead / 2, outBuffer, outBuffer.Length, false);

                        if (count > 0)
                            this.outStream.Write(outBuffer, 0, count);
                ...
                count = this.Flush(outBuffer, outBuffer.Length);
        ...
        public void ApplyID3Tag(ID3Tag tag)
        {
            if (tag == null)
                return;
        ...
        internal static class NativeMethods
		{
			...
			*/
			...
			[DllImport(libname, CallingConvention = CallingConvention.Cdecl)]
			internal static extern int lame_set_bWriteVbrTag(IntPtr context, int value);
			[DllImport(libname, CallingConvention = CallingConvention.Cdecl)]
			...
			[DllImport(libname, CallingConvention = CallingConvention.Cdecl)]
			internal static extern int lame_set_decode_only(IntPtr context, int value);
			[DllImport(libname, CallingConvention = CallingConvention.Cdecl)]
